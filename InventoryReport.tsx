...
import axiosClient from "./fontend/src/api/axiosClient";
...
const InventoryReport: React.FC = () => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await axiosClient.get("/warehouse/inventory-logs", {
        params,
      });
      setData(res.data?.data || []);

      // Tính toán sơ bộ tổng nhập/xuất trong kỳ
      const stats = (res.data?.data || []).reduce(
        (acc: any, log: InventoryLog) => {
    ...
    },
    {
      ...
      render: (val: number, record: InventoryLog) => (
        <Text
          strong
          className={
            record.type === "IMPORT" ? "text-blue-600" : "text-red-600"
          }
        >
